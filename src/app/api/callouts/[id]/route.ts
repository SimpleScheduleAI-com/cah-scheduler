...
export async function PUT(
  ...
  // All fill mutations commit atomically: a crash mid-sequence previously
  // left the callout marked "filled" with no replacement assignment row —
  // the callout page said resolved while the shift was actually uncovered.
  const updated = db.transaction(() => {
    const updatedCallout = db
      .update(callout)
      .set({
    ...
    if (updatedCallout.assignmentId) {
      const orig = db.select({ isChargeNurse: assignment.isChargeNurse })
        .from(assignment)
        .where(eq(assignment.id, updatedCallout.assignmentId))
        .get();
    ...
    if (body.replacementStaffId && updatedCallout.shiftId) {
      const shiftRecord = db
        .select({ scheduleId: shift.scheduleId, date: shift.date })
        .from(shift)
        .where(eq(shift.id, updatedCallout.shiftId))
        .get();
      ...
      if (shiftRecord) {
        ...
        // The catch keeps the surrounding transaction alive — only the insert is skipped.
        try {
          db.insert(assignment)
            .values({
              shiftId: updatedCallout.shiftId,
          ...
          logAuditEvent({
            ...
            newState: { shiftId: updatedCallout.shiftId, assignmentSource: "callout_replacement" },
          });
        } catch {
    ...
    logAuditEvent({
      ...
      newState: updatedCallout as unknown as Record<string, unknown>,
    });
